...
public class YSpringMvcContract extends SpringMvcContract {
    ...
    public YSpringMvcContract(List<AnnotatedParameterProcessor> annotatedParameterProcessors, ConversionService conversionService) {
    ...
    private String defaultValue(Method method, String pathVariable) {
        Set<ApiImplicitParam> apiImplicitParamSet = AnnotatedElementUtils.findAllMergedAnnotations(method, ApiImplicitParam.class);
        for (ApiImplicitParam apiImplicitParam : apiImplicitParamSet) {
            if (pathVariable.equals(apiImplicitParam.name())) {
                return apiImplicitParam.allowableValues().split(",")[0].trim();
            }
        }

        Set<ApiImplicitParams> apiImplicitParamsSet = AnnotatedElementUtils.findAllMergedAnnotations(method, ApiImplicitParams.class);
        for (ApiImplicitParams apiImplicitParams : apiImplicitParamsSet) {
